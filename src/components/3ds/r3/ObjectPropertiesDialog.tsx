...
export const ObjectPropertiesDialog = ({ open, onOpenChange, object, onSave }: Props) => {
  ...
  // Compact styling: smaller text, tighter rows.
  const cbCls = "flex items-center gap-1 text-[10px] leading-[13px] py-0";
  const cb = "w-[10px] h-[10px]";

  return (
    <R3Dialog open={open} onClose={() => onOpenChange(false)} title="Object Properties" width={400}>
      <div className="space-y-1 text-[10px] leading-[13px]">
        <GroupBox title="Object Information">
          <Row label="Name:" labelWidth={64}>
            <input value={name} onChange={(e) => setName(e.target.value)} className="bevel-inset bg-white text-[10px] px-1 flex-1 h-[15px]" style={{ width: 210 }} />
          </Row>
          <Row label="Dimensions:" labelWidth={64}><span className="text-[10px]">X: — Y: — Z: —</span></Row>
          <Row label="Vertices:" labelWidth={64}><span className="text-[10px]">—</span></Row>
          <Row label="Faces:" labelWidth={64}><span className="text-[10px]">—</span></Row>
          <Row label="Parent:" labelWidth={64}><span className="text-[10px]">Scene Root</span></Row>
          <Row label="Color:" labelWidth={64}>
            <label className="bevel-inset cursor-pointer inline-block" style={{ width: 32, height: 13 }} title="Object color">
            ...
            <span className="text-[10px] ml-1">{objColor}</span>
          </Row>
          <Row label="Layer:" labelWidth={64}><span className="text-[10px]">0 (default)</span></Row>
        </GroupBox>
        ...
        <div className="grid grid-cols-2 gap-1">
          <GroupBox title="Rendering Control">
            ...
            <Row label="Visibility:" labelWidth={56}>
              <Spinner value={props.visibility} onChange={(v) => set('visibility', Math.max(0, Math.min(1, v)))} step={0.05} min={0} max={1} width={44} />
            </Row>
            <Row label="G-Buffer ID:" labelWidth={64}>
              <Spinner value={props.gBufferId} onChange={(v) => set('gBufferId', Math.max(0, Math.floor(v)))} min={0} width={36} />
            </Row>
          </GroupBox>
          ...
          <GroupBox title="Display Properties">
            ...
            <Row label="Wire Color:" labelWidth={64}>
              <label className="bevel-inset cursor-pointer inline-block" style={{ width: 32, height: 13 }}>
        ...
        <GroupBox title="Motion Blur">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <Row label="Multiplier:" labelWidth={56}>
              <Spinner value={props.motionBlurMultiplier} onChange={(v) => set('motionBlurMultiplier', v)} step={0.1} width={44} />
            </Row>
          </div>
        </GroupBox>
      </div>
      ...
      <div className="mt-1 flex justify-end gap-1">
        <R3Button width={60} onClick={commit}>OK</R3Button>
